...
void testTCPSendAndReceive() {
    ...
    socketAccepted1.Receive(socketAccepted1Buffer,
                            sizeof(socketAccepted1Buffer) / sizeof(socketAccepted1Buffer[0]));
    ...
    socketAccepted2.Receive(socketAccepted2Buffer,
                            sizeof(socketAccepted2Buffer) / sizeof(socketAccepted2Buffer[0]));
    ...
    socketAccepted3.Receive(socketAcceptedBuffer3,
                            sizeof(socketAcceptedBuffer3) / sizeof(socketAcceptedBuffer3[0]));
...
void testTCPReceiveBufferSize() {
    ...
    Assert::AreEqual(4096,
                     testSocket.ReceiveBufferSize(),
                     "Size of receive buffer not equal to set value.");
#else
...
#if !defined(_WIN32) && !defined(__APPLE__)
    Assert::AreEqual(8192,
                     testSocket.SendBufferSize(),
                     "Size of send buffer not equal to set value.");
#else
    Assert::AreEqual(4096,
                     testSocket.SendBufferSize(),
                     "Size of send buffer not equal to set value.");
