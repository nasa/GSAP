 ...
 *
 *   @brief     Recorder Communicator class- records all data to a file. All data is comma seperated. 
 ...
 *                  Prognostic Results:     The output of the prognosers (from the ProgData Structure)
 ...
 *                                          .occurenceMatrix[T+%5][%6]: Occurance for time %5, sample %6
 *                                          For system trajectories the format of %0 above is the following:
 *                                          [T+%5][%4](%u): Occurance for time %5, element %4 in uData vector
 *                                              %u: Uncertainty Type
 *              Sensor data is then printed with a timestamp associated in the format "(t=%7)" in ms since 
 *                  epoch
 *              Prognostic Results are printed with a timestamp and validity in the format (v=%8, t=%7)
 *
...
#include "SharedLib.h"  // For millisecondsNow()
...
namespace PCOE {
    ...
    RecorderCommunicator::RecorderCommunicator(const ConfigMap & config) : init(false),
        ...
        if (config.includes(FILE_KEY)) {
            ...
            log.FormatLine(LOG_DEBUG, MODULE_NAME, "Configuring recorder file to %s", configFile.c_str());
        }
        ...
        if (config.includes(PROB_OCCUR_KEY)) {
            ...
            if (config.at(PROB_OCCUR_KEY)[0] == "true" || config.at(PROB_OCCUR_KEY)[0] == "1") {
                writeProbOccur = true;
                log.WriteLine(LOG_DEBUG, MODULE_NAME, "Configuring to write probability of occurence");
            }
            else {
                writeProbOccur = false;
                log.WriteLine(LOG_DEBUG, MODULE_NAME, "Configuring to NOT write probability of occurence");
            }
        }
        ...
        if (config.includes(SYS_TRAJ_KEY)) {
            ...
            if (config.at(SYS_TRAJ_KEY)[0] == "true" || config.at(SYS_TRAJ_KEY)[0] == "1") {
                ...
                writeSysTraj = false;
                log.WriteLine(LOG_DEBUG, MODULE_NAME, "Configuring to NOT write system trajectories");
            }
        }

        // Open File
        log.FormatLine(LOG_INFO, MODULE_NAME,
            "Opening data log file %s", configFile.c_str());
        ...
        if (!init) {
            ...
            for (auto & itPD : progDataMap) {
                ...
                for (auto & itEvents : itPD.second->getEventNames()) {
                    ...
                    // timeOfEvent
                    fprintf(theFile, toeFormatString,
                        progName, eventName,
                ...
                if (writeSysTraj) {
                    for (auto & itOutputs : itPD.second->getSystemTrajectoryNames()) {
                        ...
                        const auto uCert = static_cast<int>(itPD.second->sysTrajectories[itOutputs][0].uncertainty());
                        if (writePredictions) {
                            for (auto & theTime : times) {
                                fprintf(theFile, sysTrajFormatString, progName, outputName, theTime, uCert);
                            }
                        }
                        else {
                            fprintf(theFile, sysTrajFormatString, progName, outputName, times[0], uCert);
                        }
                    }  // end for each output
                }
            }
        ...
        for (auto & itPD : progDataMap) {
            ...
            for (auto & itEvents : itPD.second->getEventNames()) {
                ...
                for (unsigned long it = 0; it < vec.size(); ++it) {
                    fprintf(theFile, dataWithValidityAndTimeSpace, vec[it], timeofEventValidity,
            ...
            if (writeSysTraj) {
                for (auto & itOutputs : itPD.second->getSystemTrajectoryNames()) {
                    for (unsigned int theTime = 0; theTime < nTimes; theTime++) {
                        ...
                        const auto && trajValidity = itPD.second->sysTrajectories[itOutputs][theTime].valid();
                        const auto && trajLastUpdate = itPD.second->sysTrajectories[itOutputs][theTime].updated();
                        for (const auto & it : vec) {
                            fprintf(theFile, dataWithValidityAndTimeSpace, it, trajValidity,
