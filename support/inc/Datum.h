 ...
 *   @class Datum Datum.h
 *
 *   @brief Datum - Class for storing an individual point of data with timestamp- used with
 *                  Datastodre
...
namespace PCOE {
    ...
    public:
         ...
         *              updated to the current time whenever the value is changed using = or .set,
         *or updated to the specified time when the setTime function is used.
    ...
    template <class T>
    Datum<T>::Datum() : data(NAN), lastUpdated(std::chrono::duration<int>(0)) {}
