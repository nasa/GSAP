...
namespace PCOE {
    ...
    public:
         ...
         *parameters.
         *   @param      t Time
         *   @param      inputParameters Vector of input parameters, which are values that specify
         *how to define u for the given time.
         *   @param      u Input vector. Gets overwritten.
         **/
        virtual void inputEqn(const double t,
                              const std::vector<double>& inputParameters,
                              std::vector<double>& u) const = 0;

        /** @brief      Execute predicted output equation.
         *               Predicted outputs are those that are not measured, but are interested in
         *being predicted for prognostics.
         *   @param      t Time
         *   @param      x State vector
         *   @param      u Input vector
         *   @param      z Predicted output vector. Gets overwritten.
         **/
        virtual void predictedOutputEqn(const double t,
                                        const std::vector<double>& x,
                                        const std::vector<double>& u,
                                        std::vector<double>& z) const = 0;

        inline unsigned int getNumInputParameters() const {
            return numInputParameters;
        }
        inline unsigned int getNumPredictedOutputs() const {
            return numPredictedOutputs;
        }
