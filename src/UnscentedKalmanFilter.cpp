...
namespace PCOE {
    ...
    // Constructor
    UnscentedKalmanFilter::UnscentedKalmanFilter(Model* model, const Matrix Q, const Matrix R)
        : Observer() {
    ...
    void UnscentedKalmanFilter::step(const double newT_s,
        ...
        // Compute sigma points for current state estimate
        computeSigmaPoints(
            m_xEstimated, m_Q, m_sigmaX.kappa, m_sigmaX.alpha, m_sigmaX.M, m_sigmaX.w);
        ...
        std::vector<double> xkk1 =
            static_cast<std::vector<double>>(Xkk1.weightedMean(Matrix(m_sigmaX.w)));
        Matrix Pkk1 =
            Xkk1.weightedCovariance(Matrix(m_sigmaX.w), m_sigmaX.alpha, m_sigmaX.beta) + m_Q;
        ...
        std::vector<double> zkk1 =
            static_cast<std::vector<double>>(Zkk1.weightedMean(Matrix(m_sigmaX.w)));
        Matrix Pzz =
            Zkk1.weightedCovariance(Matrix(m_sigmaX.w), m_sigmaX.alpha, m_sigmaX.beta) + m_R;
